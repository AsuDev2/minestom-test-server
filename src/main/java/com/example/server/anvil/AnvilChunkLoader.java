...
import com.example.server.blocks.VanillaBlock;
...
public class AnvilChunkLoader implements IChunkLoader {
    ...
    private Chunk loadMCA(Instance instance, int chunkX, int chunkZ, ChunkCallback callback) throws IOException, AnvilException {
        ...
        if (mcaFile != null) {
            ...
            if (fileChunk != null) {
                ...
                batch.unsafeApply(instance, loadedChunk, c -> {
    ...
    private void loadTileEntities(Chunk loadedChunk, int chunkX, int chunkZ, Instance instance, ChunkColumn fileChunk) {
        ...
        for (NBTCompound tileEntity : fileChunk.getTileEntities()) {
            ...
            int x = tileEntity.getInt("x") + chunkX * 16;
            int y = tileEntity.getInt("y");
            int z = tileEntity.getInt("z") + chunkZ * 16;
    ...
    private void saveTileEntities(Chunk chunk, ChunkColumn fileChunk) {
        ...
        for (int index : chunk.getBlockEntities()) {
        ...
        for (int x = 0; x < Chunk.CHUNK_SIZE_X; x++) {
            for (int z = 0; z < Chunk.CHUNK_SIZE_Z; z++) {
                for (int y = 0; y < Chunk.CHUNK_SIZE_Y; y++) {
                    ...
                    BlockState state = new BlockState(block.getName(), properties);
